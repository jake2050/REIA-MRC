...
def train(args, train_dataloader):
    model = MyModel.from_pretrained(args.pretrained_model_path)
    model.train()
    if args.amp:
        scaler = GradScaler()
    ...
    for epoch in range(args.max_epochs):
        ...
        if args.test_eval and args.local_rank in [-1, 0]:
            ...
            MyLog('Turn 1: precision:{:.4f} recall:{:.4f} f1:{:.4f}'.format(p1, r1, f1))
            MyLog("Turn 2: precision:{:.4f} recall:{:.4f} f1:{:.4f}".format(p2, r2, f2))
            model.train()
        if args.local_rank != -1:
