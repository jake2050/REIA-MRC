...
class BertDeLayer(nn.Module):
    ...
    def forward(self, chidden_states, qhidden_states, attention_mask, qattention_mask):
        ...
        cattention_output, qattention_output = self.attention(chidden_states, qhidden_states, attention_mask,
                                                              qattention_mask)
        cintermediate_output, qintermediate_output = self.intermediate(cattention_output, qattention_output)
        clayer_output, qlayer_output = self.output(cintermediate_output, qintermediate_output, cattention_output,
                                                   qattention_output)
...
class BertDirectedAttention(nn.Module):
    ...
    def forward(self, hidden_states, cattention_mask, qattention_mask, output_all_deencoded_layers=True):
        ...
        for layer_module in self.layer:
            chidden_states, qhidden_states = layer_module(chidden_states, qhidden_states, cattention_mask,
                                                          qattention_mask)
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        cextended_attention_mask = cextended_attention_mask.to(
        ...
        qextended_attention_mask = qextended_attention_mask.to(
            dtype=next(self.parameters()).dtype)  # fp16 compatibility
...
class MyModel(PreTrainedBertModel):
    def __init__(self, config):
        super(MyModel, self).__init__(config)
        # self.bert = BertModel.from_pretrained(config.pretrained_model_path)
        self.bert = BertModel(config)
        self.tag_linear = nn.Linear(config.hidden_size, 5)
        ...
        self.apply(self.init_bert_weights)
        self.conv1d_1 = nn.Conv1d(in_channels=2 * config.hidden_size, out_channels=3 * config.hidden_size // 2,
                                  kernel_size=3, padding=1)
        self.conv1d_2 = nn.Conv1d(in_channels=3 * config.hidden_size // 2, out_channels=config.hidden_size,
                                  kernel_size=3, padding=1)
    ...
    def forward(self, input, attention_mask, token_type_ids, context_mask=None, turn_mask=None, target_tags=None):
        ...
        """
        extended_attention_mask, c_attention_mask, q_attention_mask, sequence_output = self.bert(input,
                                                                                                 token_type_ids,
                                                                                                 attention_mask,
                                                                                                 output_all_encoded_layers=False)# sequence_output.size = [B, L, D]
        sequence_output = self.dropout(sequence_output)
        cdeencoded_layers, qdeencoded_layers = self.decoder(sequence_output,  # 2d --> 1d translated
                                                            c_attention_mask, q_attention_mask,
                                                            output_all_deencoded_layers=False)# cdeencoded_layers.shape = [B, L_c, 768]
                                                                                              # qdeencoded_layers.shape= [B, L, 768]
        ...
        enc_cat = torch.cat((cdeencoded_layers, qdeencoded_layers), dim=-1)# [10, L, 768, 2]

        encshape = enc_cat.shape# torch.Size([10, L, 768, 2])
        enc_cat = enc_cat.reshape(encshape[0], encshape[1], -1).contiguous()
        enc_cat = enc_cat.permute(0, 2, 1).contiguous()# [10, 1536, L]
        ...
        tag_logits = self.tag_linear(sequence_output1d)  # (batch,seq_len,num_tag)
